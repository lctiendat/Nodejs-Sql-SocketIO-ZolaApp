...
function getGroupByCode(groupCode) {
    return new Promise((res, rej) => {
        connection.query(`SELECT name, (SELECT COUNT(*) FROM members_of_group WHERE group_code = '${groupCode}' ) as countMemBer FROM groups WHERE code = '${groupCode}' AND delete_flag = 0
        `, (err, row) => {
            if (err) return rej(err)
            res(row)
        })
...
function getListFriendNotInGroup(groupCode, userEmail) {
    return new Promise((res, rej) => {
        ...
        WHERE email IN 
        (SELECT friendEmail as email FROM friends WHERE userEmail = '${userEmail}' AND status = 'accept' AND delete_flag = 0 union SELECT userEmail as email FROM friends WHERE friendEmail = '${userEmail}' AND status = 'accept' AND delete_flag = 0 ) AND email NOT IN (SELECT email FROM members_of_group WHERE group_code= '${groupCode}' ) ORDER BY id DESC
`, (err, row) => {
            if (err) return rej(err)
            res(row)
        })
